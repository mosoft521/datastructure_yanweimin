...
int succ(int i) { /* 求后继函数 */
    return ++i;
}

void Collect(SLCell r[], ArrType f, ArrType e) { /* 本算法按keys[i]自小至大地将f[0..RADIX-1]所指各子表依次链接成 */
...
void RadixSort(SLList *L) { /* L是采用静态链表表示的顺序表。对L作基数排序，使得L成为按关键字 */

...
void DFSTree(ALGraph G, int v, CSTree *T) { /* 从第v个顶点出发深度优先遍历图G,建立以T为根的生成树。算法7.8 */
    Boolean first = TRUE;
    int w;
    CSTree p, q;
    VertexType v1, w1;
...
void DFSForest(ALGraph G, CSTree *T) { /* 建立无向图G的深度优先生成森林的(最左)孩子(右)兄弟链表T。算法7.7 */
    CSTree p, q = NULL;
    int v;
    *T = NULL;
...
void PreOrderTraverse(CSTree T, void(*Visit)(TElemType)) { /* 先根遍历孩子－兄弟二叉链表结构的树T(bo6-5.c改) */
    if (T) {
        Visit(T->data); /* 先访问根结点 */

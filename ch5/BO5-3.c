...
Status CopySMatrix(RLSMatrix M, RLSMatrix *T) { /* 由稀疏矩阵M复制得到T */
    *T = M;
    return OK;
}
...
Status AddSMatrix(RLSMatrix M, RLSMatrix N, RLSMatrix *Q) { /* 求稀疏矩阵的和Q=M+N */
    ...
    for (k = 1; k <= M.mu; ++k) /* 对于每一行，k指示行号 */
    {
        ...
        while (p < M.rpos[k + 1] && q < N.rpos[k + 1]) { /* M,N矩阵均有第k行元素未处理 */
...
Status SubtSMatrix(RLSMatrix M, RLSMatrix N, RLSMatrix *Q) { /* 求稀疏矩阵的差Q=M-N */
    int i;
    if (M.mu != N.mu || M.nu != N.nu)
        return ERROR;
...
Status MultSMatrix(RLSMatrix M, RLSMatrix N, RLSMatrix *Q) { /* 求稀疏矩阵乘积Q=M＊N。算法5.3 */
...
Status TransposeSMatrix(RLSMatrix M, RLSMatrix *T) { /* 求稀疏矩阵M的转置矩阵T */
    int p, q, t, col, *num;

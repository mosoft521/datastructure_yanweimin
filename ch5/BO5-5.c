...
Status CopyGList(GList *T, GList L) { /* 采用头尾链表存储结构,由广义表L复制得到广义表T。算法5.6 */
    if (!L) /* 复制空表 */
        *T = NULL;
    else {
...
int GListLength(GList L) { /* 返回广义表的长度,即元素个数 */
    int len = 0;
    if (!L)
        return 0;
    if (L->tag == ATOM)
...
Status GListEmpty(GList L) { /* 判定广义表是否为空 */
    if (!L)
        return TRUE;
    else
        return FALSE;
...
Status DeleteFirst_GL(GList *L, GList *e) { /* 初始条件: 广义表L存在 */

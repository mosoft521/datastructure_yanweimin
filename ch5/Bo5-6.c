...
#include "../ch4/c4-2.h" /* 定义HString类型 */
#include "../ch4/bo4-2.c" /* HString类型的基本操作 */
...
Status CreateGList(GList *L, HString S) { /* 初始条件: S是广义表的书写形式串。操作结果: 由S创建广义表L */
...
void DestroyGList(GList *L) { /* 初始条件: 广义表L存在。操作结果: 销毁广义表L */
    GList ph, pt;
    if (*L) /* L不为空表 */
    { /* 由ph和pt接替L的两个指针 */
        if ((*L)->tag) /* 是子表 */
...
Status CopyGList(GList *T, GList L) { /* 初始条件: 广义表L存在。操作结果: 由广义表L复制得到广义表T */
    if (!L) /* L空 */
    {
        *T = NULL;
        return OK;
...
int GListLength(GList L) { /* 初始条件: 广义表L存在。操作结果: 求广义表L的长度,即元素个数 */
    int len = 0;
    GList p;
...
int GListDepth(GList L) { /* 初始条件: 广义表L存在。操作结果: 求广义表L的深度 */
    int max, dep;
    GList pp;
...
Status GListEmpty(GList L) { /* 初始条件: 广义表L存在。操作结果: 判定广义表L是否为空 */
...
GList GetHead(GList L) { /* 初始条件: 广义表L存在。操作结果: 取广义表L的头 */
    GList h;
    InitGList(&h);
    if (!L || L->tag == LIST && !L->a.hp) {
        printf("\n空表无表头!");
...
GList GetTail(GList L) { /* 初始条件: 广义表L存在。操作结果: 取广义表L的尾 */
    GList T;
    if (!L) {
        printf("\n空表无表尾!");
        exit(0);
...
Status InsertFirst_GL(GList *L, GList e) { /* 初始条件: 广义表存在 */
...
Status DeleteFirst_GL(GList *L, GList *e) { /* 初始条件:广义表L存在。操作结果:删除广义表L的第一元素,并用e返回其值 */
    if (*L) {
        *e = (*L)->a.hp;
        (*L)->a.hp = (*e)->tp;
        (*e)->tp = NULL;
...
void Traverse_GL(GList L, void(*v)(AtomType)) { /* 利用递归算法遍历广义表L */
    GList hp;
    if (L) /* L不空 */
    {
        if (L->tag == ATOM) /* L为单原子 */

...
void DestroyBiTree() { /* 由于SqBiTree是定长类型,无法销毁 */
}
...
Status InsertChild(SqBiTree T, TElemType p, Status LR, SqBiTree c) { /* 初始条件: 二叉树T存在,p是T中某个结点的值,LR为0或1,非空二叉树c与T */
    /*           不相交且右子树为空 */
...
#include "../ch3/c3-3.h" /* 顺序非循环队列 */
#include "../ch3/bo3-4.c" /* 顺序非循环队列的基本操作 */

Status DeleteChild(SqBiTree T, position p, int LR) { /* 初始条件: 二叉树T存在,p指向T中某个结点,LR为1或0 */
...
Status InOrderTraverse(SqBiTree T, Status(*Visit)(TElemType)) { /* 初始条件: 二叉树存在,Visit是对结点操作的应用函数 */
...
Status PostOrderTraverse(SqBiTree T, Status(*Visit)(TElemType)) { /* 初始条件: 二叉树T存在,Visit是对结点操作的应用函数 */

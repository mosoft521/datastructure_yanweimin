...
void InitIdxList(IdxListType *idxlist) { /* 初始化操作，置索引表idxlist为空表，且在idxliat.item[0]设一空串 */
    (*idxlist).last = 0;
    InitString(&(*idxlist).item[0].key); /* bo4-2.c */
    InitList(&(*idxlist).item[0].bnolist); /* bo2-6.c */
}

void ExtractKeyWord(int *BookNo) { /* 从buf中提取书名关键词到词表wdlist，书号存入BookNo */
...
int Locate(IdxListType *idxlist, HString wd, Status *b) { /* 在索引表idxlist中查询是否存在与wd相等的关键词。若存在,则返回其 */
...
void InsertBook(IdxListType *idxlist, int i, int bno) { /* 在索引表idxlist的第i项中插入书号为bno的索引 */
    Link p;
...
void InsIdxList(IdxListType *idxlist, int bno) { /* 将书号为bno的关键词插入索引表 */
    int i, j;
    Status b;
    HString wd;
...
void PutText(FILE *f, IdxListType idxlist) { /* 将生成的索引表idxlist输出到文件f */
    int i, j;
    Link p;
    fprintf(f, "%d\n", idxlist.last);

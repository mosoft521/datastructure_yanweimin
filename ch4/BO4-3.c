...
Status StrAssign(LString *T, char *chars) { /* 生成一个其值等于chars的串T(要求chars中不包含填补空余的字符) */
    /* 成功返回OK，否则返回ERROR */
    int i, j, k, l;
    Chunk *p, *q = NULL;
...
Status StrCopy(LString *T, LString S) { /* 初始条件:串S存在。操作结果:由串S复制得串T(连填补空余的字符一块拷贝) */
    Chunk *h = S.head, *p, *q;
    (*T).curlen = S.curlen;
    if (h) {
...
Status Concat(LString *T, LString S1, LString S2) { /* 用T返回由S1和S2联接而成的新串 */
    LString a1, a2;
    InitString(&a1);
    InitString(&a2);
    StrCopy(&a1, S1);
...
int Index(LString S, LString T, int pos) { /* T为非空串。若主串S中第pos个字符之后存在与T相等的子串, */
...
void Zip(LString *S) { /* 压缩串(清除块中不必要的填补空余的字符)。加 */
    int j, n = 0;
    Chunk *h = (*S).head;
    char *q;
...
Status Replace(LString *S, LString T, LString V) { /* 初始条件: 串S,T和V存在,T是非空串（此函数与串的存储结构无关） */
...
void StrPrint(LString T) { /* 输出字符串T。另加 */
    int i = 0, j;
    Chunk *h;
    h = T.head;
    while (i < T.curlen) {

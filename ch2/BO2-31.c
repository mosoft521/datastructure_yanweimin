...
void InitList(SLinkList L) { /* 构造一个空的链表，表头为L的最后一个单元L[MAXSIZE-1]，其余单元链成 */
...
Status ClearList(SLinkList L) { /* 初始条件：线性表L已存在。操作结果：将L重置为空表 */
    int i, j, k;
...
Status ListEmpty(SLinkList L) { /* 若L是空表，返回TRUE；否则返回FALSE */
...
int ListLength(SLinkList L) { /* 返回L中数据元素个数 */
...
Status GetElem(SLinkList L, int i, ElemType *e) { /* 用e返回L中第i个元素的值 */
...
Status PriorElem(SLinkList L, ElemType cur_e, ElemType *pre_e) { /* 初始条件：线性表L已存在 */
...
Status NextElem(SLinkList L, ElemType cur_e, ElemType *next_e) { /* 初始条件：线性表L已存在 */
...
Status ListDelete(SLinkList L, int i, ElemType *e) { /* 删除在L中第i个数据元素e，并返回其值 */
...
Status ListTraverse(SLinkList L, void(*vi)(ElemType)) { /* 初始条件:线性表L已存在 */

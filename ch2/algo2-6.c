...
void Print(struct stud e) { /* 显示记录e的内容 */
...
void ReadIn(struct stud *e) { /* 由键盘输入结点信息 */
...
void WriteToFile(struct stud e) { /* 将结点信息写入fp指定的文件 */
    fwrite(&e, sizeof(struct stud), 1, fp);
}

Status ReadFromFile(struct stud *e) { /* 由fp指定的文件读取结点信息到e */
    int i;
...
Status FindFromNum(LinkList L, long num, LinkList *p, LinkList *q) { /* 查找表中学号为num的结点,如找到,q指向此结点,p指向q的前驱, */
...
Status FindFromName(LinkList L, char name[], LinkList *p, LinkList *q) { /* 查找表中姓名为name的结点,如找到,q指向此结点,p指向q的前驱, */
    /* 并返回TRUE;如无此元素,则返回FALSE */
    *p = L;
    while (*p) {
        *q = (*p)->next;
...
Status DeleteElemNum(LinkList L, long num) { /* 删除表中学号为num的元素，并返回TRUE；如无此元素，则返回FALSE */
    LinkList p, q;
...
Status DeleteElemName(LinkList L, char name[]) { /* 删除表中姓名为name的元素，并返回TRUE；如无此元素，则返回FALSE */
    LinkList p, q;

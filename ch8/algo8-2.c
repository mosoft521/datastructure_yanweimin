...
Space AllocBuddy(FreeList *avail, int n) { /* avail[0..m]为可利用空间表,n为申请分配量,若有不小于n的空闲块, */
...
Space buddy(Space p) { /* 返回相对起始地址为p,块大小为pow(2,k)的块的伙伴地址 */
...
void Reclaim(FreeList *pav, Space *p) { /* 伙伴系统的回收算法 将p所指的释放块回收到可利用空间表pav中 */
    Space s;
    s = buddy(*p); /* 伙伴块的起始地址 */
...
void Print(FreeList p) { /* 输出p中所有可利用空间表 */
    int i;
    Space h;
    for (i = 0; i <= m; i++) {
